...
if __name__ == '__main__':
    table = {}
    # with open('linggle_mp_pat.txt','w') as output_pat_file:

                                            # Eric 2016/6/12
    linggle_dict = defaultdict( lambda: defaultdict( lambda : 0 ) )
    # build_linngle_dict()
    #generate linggle dict
    with open('linggle_mp_pat_na.txt','w') as linggle_pattern_na:
        for line in fileinput.input():
        #if n/a, then record
            if line.strip().split('\t')[1] == 'N/A':
                linggle_pattern_na.write( '{}\n'.format( line.strip().split('\t')[0].replace('n.','something') ) )

                continue
            linggle_dict[line.strip().split('\t')[0].replace('n.','something')][line.strip().split('\t')[1]] = float(line.strip().split('\t')[2])

    # print linggle_dict

    #start disambuiguate process
    for pat in linggle_dict:
        # print '\t'+pat
        # adjusted_pad is used for reassemble the pattern
        adjusted_pat = pat.split()

        #get positions of 'something'
        #something may appear many times
        ...
        #get coll in list       
        colls = []
        for coll in linggle_dict[pat]:
            colls += [coll]

        # maybe have a lot of sth, we need to disambiguate every sth
        # so we will process every word which is at the same position of sth
        for every_sth_word in sth_position:
            # group_num = sth_position.index(every_sth_word)
            # print '\tProcessing position of sth :' + str(every_sth_word)

            # each sth word's count
            coll_count_dict = defaultdict(lambda : 0)

            # get each corresponding word which is at the same position of sth
            sth_words = []
            for coll in colls:
                ...
                coll_count_dict[pattern_seperate[every_sth_word]] += linggle_dict[pat][coll]
        ...
        # after get every position's new class
        #  we generate the new pat
        # for every_sth_word in sth_position:

        generate_dead_new_pat()
